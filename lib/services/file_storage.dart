...
class FileStorage {
  ...
  static Future<void> initialise() async {
    ...
    if (Platform.isAndroid) {
      directory = Directory('/storage/emulated/0/Download/Shrayesh Music');
    } else if (Platform.isWindows) {
      directory =
          Directory(path.join((await getDownloadsDirectory())!.path, 'shreeya'));
    } else {
  ...
  Future<void> loadBackup(BuildContext context) async {
    ...
    if (backup['name'] != 'shreeya' && backup['type'] != 'backup') {
      return;
    }
